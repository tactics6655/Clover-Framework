	...
	A = core.detectAdblock = {
		...
		detect: function () {
			if ($.core.Validate.isUndefined($cache['adblock'])) {
				...
				if (
					_cWin.document.body.getAttribute('abp') !== null || 
					pAdBlocker.offsetParent === null || 
					pAdBlocker.offsetHeight == 0 || 
					pAdBlocker.offsetLeft == 0 || 
					pAdBlocker.offsetTop == 0 || 
					pAdBlocker.offsetWidth == 0 || 
					pAdBlocker.clientHeight == 0 || 
					pAdBlocker.clientWidth == 0) {
					detected = true;
				}
